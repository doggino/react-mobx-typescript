...
export default class OptionsPanel extends Component<IOptionsPanelStore> {
  ...
  private limitInputHandle = (e: React.ChangeEvent<HTMLInputElement>): void => {
    this.props.optionsPanelStore!.setLimit(
      +e.target.value > 0 ? +e.target.value : 1
    );
  };
  private offSetInputHandle = (
    e: React.ChangeEvent<HTMLInputElement>
  ): void => {
    this.props.optionsPanelStore!.setOffset(
      +e.target.value > 0 ? +e.target.value : 0
    );
  };
  private ratingInputHandle = (
  ...
  render() {
    const { type, limit, offset, rating, lang } = this.props.optionsPanelStore!;
    return (
      <div className={styles.options}>
        ...
        <label>
          ...
          <input
            type="number"
            value={offset}
            onChange={this.offSetInputHandle}
          />
        </label>
        <label>
          Rating:
